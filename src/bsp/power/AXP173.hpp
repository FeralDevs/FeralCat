...
class AXP173_Class : public I2C_Device
{
    private:
    ...
    public:
        // Constructor: Initialize with I2C address and default bus
        AXP173_Class(I2C_Class* i2c = &In_I2C) 
            : I2C_Device(AXP173_ADDR, 400000, i2c) {}
        ...
        //地址：0x00
        bool isACINExist() const;      // ACIN存在指示
        bool isACINAvl() const;        // ACIN是否可用
        bool isVBUSExist() const;      // VBUS存在指示
        bool isVBUSAvl() const;        // VBUS是否可用
        bool getBatCurrentDir() const; //获取电池电流方向（0：在放电；1：在充电）
        //地址：0x01
        bool isAXP173OverTemp() const; //指示AXP173是否过温
        bool isCharging() const;       //充电指示（0：未充电或已充电完成；1：正在充电）
        bool isBatExist() const;       //电池存在状态指示
        bool isChargeCsmaller() const; //指示充电电流是否小于期望电流（0：实际充电电流等于期望电流；1：实际充电电流小于期望电流）
        ...
        /* PWROK control */
        void setPWROKDelay(uint8_t delay_ms);        //设置PWROK延迟时间
        void setVoffVoltage(uint16_t voltage);       //设置关机电压
        ...
        uint32_t getCoulometerChargeData(void) const;    //电池充电库仑计数据寄存器3
        uint32_t getCoulometerDischargeData(void) const; //电池放电库仑计数据寄存器3
        float GetBatCoulombInput(void) const;            //库仑计输入计数
        float GetBatCoulombOutput(void) const;           //库仑计输出计数
        float getCoulometerData(void) const;             //计算后返回的值 get coulomb val affter calculation

        /* BAT data (电池状态数据) */
        float getBatVoltage() const; //返回高八位 + 低四位电池电压   地址：高0x78 低0x79
        float getBatCurrent() const; //返回高八位 + 低五位电池电流   地址：充电电流（高0x7A 低0x7B） & 放电电流（高0x7C 低0x7D）
        float getBatLevel() const;   //返回电池电量等级（%）
        float getBatPower() const;   //返回高八位 + 中八位 + 低八位电池瞬时功率  地址：高0x70 中0x71 低0x72
        // uint32_t getChargeTimeMS();

        /* VBUS data (外部输入电压状态数据) */
        float getVBUSVoltage() const; //返回高八位 + 低四位USB输入电压   地址：高0x5A 低0x5B
        float getVBUSCurrent() const; //返回高八位 + 低四位USB输入电流   地址：高0x5C 低0x5D

        /* Temperature data (温度监控数据) */
        float getAXP173Temp() const; //返回高八位 + 低四位芯片内置温度传感器温度 地址：高0x5E 低0x5F
        float getTSTemp() const;     //返回高八位 + 低四位芯片TS脚热敏电阻检测到的电池温度  地址：高0x62 低0x63
        void setTSCurrent(uint8_t chargingUA, uint8_t notChargingUA); //设置TS引脚电流 (20/40/60/80μA)

...
// ── I2C_Class based register access (C++ linkage) ──

static I2C_Class* _i2c = &In_I2C;

static const char *TAG __attribute__((unused)) = "ES8311";

void es8311_set_i2c(I2C_Class* i2c)
{
    _i2c = i2c;
}
...
esp_err_t es8311_write_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t data)
{
    ...
    if (!_i2c->writeRegister8(es->dev_addr, reg_addr, data)) {
        return ESP_FAIL;
    }
    return ESP_OK;
}
...
{
    es8311_dev_t *es = (es8311_dev_t *) dev;
    *reg_value = _i2c->readRegister8(es->dev_addr, reg_addr);
    return ESP_OK;
}

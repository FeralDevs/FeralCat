...
MP3 Player **1.1.1** appears as its own tile in the normal app menu. It requires
firmware with the native player UI, provided by **v0.8.1-lua.2**, based on upstream
...
remains hidden by default. App menus, controls, status text, and errors are in
English.
...
player tile; use **App manager → Rescan SD apps** if needed. Inside the player,
**Library → More options → Rescan music** refreshes the music catalog. The
firmware does not simultaneously play from the card while USB mass-storage
mode owns it.
...
- **Library:** offers all tracks or M3U playlists. Selecting a track starts the
  complete selected list at that position. Two tracks are shown
  per page. Use the page button or left/right to browse; the last page offers
  **First page**.
- **Sound → Volume:** the **Quieter -5** and **Louder +5** buttons change
...
- **Sound → Equalizer:** offers Neutral, Voice, Warm, and Small speaker over
  two pages. Presets use fixed band attenuation; arbitrary bass/treble boosts
  are not provided.
- **Sound → Playback options:** offers shuffle and repeat. **Seek / Stop**
  contains ten-second seeks and stop.
- **Back or short B:** returns from a subpage without restarting
...
- **Long B:** exits the app and stops playback. For touch exit, use
  **Library → More options → Exit player**, then confirm.
  **Keep listening** cancels exit.
...
**Repeat: Off** ends playback after the last entry. **Repeat: All** continues
from the start of the queue. **Repeat: One** repeats the current track.

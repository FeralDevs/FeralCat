...
{
   /* The 6502 address bus wraps at $FFFF, including operand fetches. */
   const uint16 base = (uint16) address;
   return mem_getbyte((uint16) (base + 1)) << 8 | mem_getbyte(base);
}
...
mem_t *mem_init_(void)
{
   ...
   if (!mem.dummy) return NULL;
   memset(mem.dummy, 0xFF, MEM_PAGESIZE);
   mem_reset();
   return &mem;
}

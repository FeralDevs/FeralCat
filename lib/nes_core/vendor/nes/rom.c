...
rom_t *rom_loadmem(uint8 *data, size_t size)
{
   ...
   if (!memcmp(data, ROM_NES_MAGIC, 4))
   {
      ...
      /* Header byte 8 is in 8 KiB units; legacy zero means one.
       * Exact DiskDude overwrites this byte and is treated as legacy. */
      rom.prg_ram_banks = memcmp(data + 7, "DiskDude!", 9) == 0 ? 1 : (data[8] ? data[8] : 1);
   ...
   rom.prg_ram = calloc(rom.prg_ram_banks, ROM_PRG_BANK_SIZE);
   rom.chr_ram = calloc(rom.chr_ram_banks, ROM_CHR_BANK_SIZE);
   if (!rom.prg_rom)
   {

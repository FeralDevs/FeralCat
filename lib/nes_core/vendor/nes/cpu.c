...
static inline uint16 zp_readword(uint8 addr)
{
   /* Indirect zero-page pointers wrap within page zero, not into $0100. */
   return cpu.zp[addr] | ((uint16) cpu.zp[(uint8) (addr + 1)] << 8);
}
#define ZP_READWORD(addr)          (zp_readword(addr))
...
static inline uint8 fast_readbyte(uint16 a) { return cpu.pages[a >> MEM_PAGESHIFT][a]; }
static inline uint16 fast_readword(uint16 a) {
   return ((a & MEM_PAGEMASK) != MEM_PAGEMASK) ? PAGE_READWORD(cpu.pages[a >> MEM_PAGESHIFT], a) : mem_getword(a);
}
...
#define OPCODE(xx, ...)  op##xx: __VA_ARGS__; OPCODE_NEXT
#define OPCODE_NEXT \
...
#define OPCODE(xx, ...)  case xx: __VA_ARGS__; break;
#define OPCODE_NEXT \
   DISASSEMBLE;  \

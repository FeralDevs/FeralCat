...
static symbol_resolver current_resolver = elf_find_sym_default; /* de-atomic: GCC8.4 + fn-ptr _Atomic; set once */
...
static int esp_elf_load_section(esp_elf_t *elf, const uint8_t *pbuf)
{
    ...
    if (size) {
        ...
        if (!elf->pdata) {
            ...
            elf->ptext = NULL; /* Caller deinitializes even after relocate fails. */
            return -ENOMEM;
        }
    }
...
{
    current_resolver = elf_find_sym_default;
}

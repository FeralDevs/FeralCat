...
{
    std::string value;
    CHECK(normalize("/music/album", "..\\mix/./Song.MP3", &value));
    CHECK(value == "/music/mix/Song.MP3");
    CHECK(normalize("/music/a/b", "../../track.mp3", &value) && value == "/music/track.mp3");
    CHECK(normalize("/music/ignored", "/MUSIC//a/../b.mp3", &value) && value == "/music/b.mp3");
    const std::vector<std::string> bad = {"", "../outside.mp3", "a/../../outside.mp3",
      "/music0/track.mp3", "/mp3/track.mp3", "/music/../music/track.mp3",
      "https://example/a.mp3", "C:\\mp3\\a.mp3", "file:///music/a.mp3",
    ...
    for (const auto& text : bad) CHECK(!normalize("/music", text));
    CHECK(!normalize("/outside", "track.mp3"));
    CHECK(!normalize("/music/../outside", "track.mp3"));
    const std::string unicode = "Gr\xc3\xbc\xc3\x9f" "e-\xf0\x9f\x90\xb1.mp3";
    CHECK(normalize("/music", unicode, &value) && value == "/music/" + unicode);
    CHECK(validMediaText(unicode.data(), unicode.size()));
    CHECK(normalize("/music", std::string(180, (char)0x61) + ".mp3", &value) && value.size() == 191);
    CHECK(!normalize("/music", std::string(181, 'a') + ".mp3"));
    char tiny[4] = "x";
    CHECK(!normalizeMediaPath("/music", "a.mp3", 5, tiny, sizeof(tiny)) && tiny[0] == 0);
    CHECK(!normalizeMediaPath("/music", "a", 1, nullptr, 0));
    CHECK(compareMediaPaths("/music/A.mp3", "/MUSIC/a.MP3") == 0);
    CHECK(compareMediaPaths("/music/b.mp3", "/music/A.mp3") > 0);
}

static void scanAndPlaylistTests()
{
    mock::Disk disk;
    disk.file("/music/Zulu.mp3"); disk.file("/music/alpha.MP3");
    disk.file("/music/album/beta.mp3"); disk.file("/music/note.txt");
    disk.file("/music/d1/d2/d3/d4/allowed.mp3");
    disk.file("/music/d1/d2/d3/d4/d5/too-deep.mp3");
    const std::string longStem = std::string(94, 'a') + "\xf0\x9f\x90\xb1";
    disk.file("/music/" + longStem + ".mp3");
    ...
    disk.file("/music/album/Mix.m3u8", playlist);
    disk.file("/music/A.m3u", "alpha.MP3\n");
    ...
    const uint16_t beta = idFor(catalog, "/music/album/beta.mp3");
    const uint16_t zulu = idFor(catalog, "/music/Zulu.mp3");
    const uint16_t alpha = idFor(catalog, "/music/alpha.MP3");
...
static void limitsTests()
{
    {
        mock::Disk disk;
        for (size_t i = 0; i < 520; ++i) disk.file("/music/" + number(519 - i) + ".mp3");
        std::string entries;
        for (size_t i = 0; i < 515; ++i) entries += "0000.mp3\n";
        for (size_t i = 0; i < 17; ++i) disk.file("/music/" + number(i) + ".m3u", entries);
        fs::FS source(disk); MediaCatalog catalog;
        CHECK(catalog.scan(source) == MediaCatalog::ScanResult::Complete);
        CHECK(catalog.trackCount() == 512 && catalog.playlistCount() == 16);
        char path[192], title[96];
        CHECK(catalog.track(1, path, sizeof(path), title, sizeof(title)) && std::string(path) == "/music/0000.mp3");
        CHECK(catalog.track(512, path, sizeof(path), title, sizeof(title)) && std::string(path) == "/music/0511.mp3");
        ...
        std::reverse(disk.nodes["/music"].children.begin(), disk.nodes["/music"].children.end());
        CHECK(catalog.scan(source) == MediaCatalog::ScanResult::Complete);
        CHECK(catalog.track(1, path, sizeof(path), title, sizeof(title)) && std::string(path) == "/music/0000.mp3");
        CHECK(catalog.track(512, path, sizeof(path), title, sizeof(title)) && std::string(path) == "/music/0511.mp3");
    ...
    {
        mock::Disk disk; disk.file("/music/a.mp3");
        disk.file("/music/large.m3u", std::string(MaxPlaylistBytes + 1, '\n'));
        fs::FS source(disk); MediaCatalog catalog;
        CHECK(catalog.scan(source) == MediaCatalog::ScanResult::Complete);
    ...
    }
    {
        ...
        disk.file("/music/exact.m3u", exact);
        fs::FS source(disk); MediaCatalog catalog;
        CHECK(catalog.scan(source) == MediaCatalog::ScanResult::Complete);
    ...
    {
        mock::Disk disk;
        for (size_t i = 0; i < 4100; ++i) disk.file("/music/" + number(i) + ".txt");
        fs::FS source(disk); MediaCatalog catalog;
        CHECK(catalog.scan(source) == MediaCatalog::ScanResult::Complete);
    ...
    {
        mock::Disk disk; disk.file("/music/valid.mp3");
        disk.file("/music/bad.mp3"); disk.nodes["/music/bad.mp3"].leafOverride = "../outside.mp3";
        disk.file("/music/A.mp3"); disk.file("/music/a.MP3");
        fs::FS source(disk); MediaCatalog catalog;
        CHECK(catalog.scan(source) == MediaCatalog::ScanResult::Complete);
...
static void cancellationAndFailureTests()
{
    ...
    disk.file("/music/mix.m3u", entries);
    fs::FS source(disk);
    {
    ...
    mock::heap = {};
    {
        ...
        CHECK(std::strstr(catalog.error(), "/music") && mock::heap.live == 0 && missing.live == 0);
    }
    {
